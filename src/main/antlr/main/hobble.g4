...
//parser
program returns [Expr ret] : { List<Expr>lines=new ArrayList<Expr>(); } (code ';'? { lines.add($code.ret); })*
    { $ret=new Block(lines, false); System.out.println("------------------v Output v------------------"); } EOF;
...
//interrupts (10+)
    | { Expr expr=new NoneExpr(); } 'return' (code {expr=$code.ret; })? { $ret=new Interrupt(0, expr); }
